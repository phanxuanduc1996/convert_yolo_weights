...
def parse_arguments(argv):
    ...
    parser.add_argument('--mlmodel_path', help='Path to output ML-Model file.',
                        default='../../pretrain_models/yolov3.mlmodel')
...
def convert_h5_to_mlmodel():
    ...
    coreml_model = coremltools.converters.keras.convert(args.h5_path, input_names=['image'], output_names=[
        'output1', 'output2', 'output3'], image_scale=1 / 255., class_labels=output_labels, image_input_names='image', input_name_shape_dict={'image': [None, args.img_size, args.img_size, 3]})
    # Case 2:
    # coreml_model = coremltools.converters.keras.convert(args.h5_path, input_names=['image'], output_names=[
    ...
    # YOLO V2
    # coreml_model = coremltools.converters.keras.convert(args.h5_path, input_names=['image'], output_names=[
    #     'grid'], image_scale=1/255., class_labels=output_labels, image_input_names='image', input_name_shape_dict={'image': [None,  args.img_size,  args.img_size, 3]})
