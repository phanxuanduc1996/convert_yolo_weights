...
def predict_with_yolo_head(model, orig, config, confidence=0.5, iou_threshold=0.4):
    ...
    predictions = yolo_head(model.predict([image_data]), num_classes=10,
...
def draw_boxes(image, boxes, classes, scores, config):
    ...
    confidences = []
    for key in sorted(confidence.keys()):
        confidences.append(confidence[key])
    #print("shape:", image.shape)
    #return image
    #cv2.imwrite("out.png", image)
    return predict, confidences
