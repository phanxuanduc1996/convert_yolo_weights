...
class YOLONMSLayer(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        nms_final_ = K.concatenate(
            [batch_indexs_, classes_1, class_box_range_], axis=1)
...
class YOLO(object):
    def __init__(self, model_path='../../pretrain_models/yolov3.h5', anchors_path='model_data/yolo_anchors.txt', yolo3_dir=None):
        self.yolo3_dir = yolo3_dir
        self.model_path = model_path
        self.anchors_path = anchors_path
        self.classes_path = 'model_data/coco_classes'
        self.score = 0.3
        self.iou = 0.45
    ...
    def load_model(self, yolo_weights=None):
        ...
        image_input = keras.Input(
            (None, None, 3), dtype='float32', name='input_1')
        y = list(self.yolo_model(image_input))
        y.append(input_image_shape)

        boxes, box_scores = \
            YOLOEvaluationLayer(anchors=self.anchors,
                                num_classes=len(self.class_names))(inputs=y)
    ...
    def detect_with_onnx(self, image):
        ...
        all_boxes_k, all_scores_k, indices_k = self.final_model.predict(
        ...
        all_boxes, all_scores, indices = self.session.run(
            None, input_feed=feed_f)
...
def convert_model(yolo, model_file_name, target_opset):
    yolo.load_model()
    onnxmodel = convert_keras(
        yolo.final_model, target_opset=target_opset, channel_first_inputs=['input_1'])
...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_file_name):
        onnxmodel = convert_model(
            YOLO(model_path, anchors_path), model_file_name, target_opset)
