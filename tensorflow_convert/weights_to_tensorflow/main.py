...
def parse_net(num_layers, cfg, weights, training=False, const_inits=True, verbose=True):
    ...
    for ith, layer in enumerate(cfg_walker):
        ...
        scope = "{}{}{}".format(
            args.prefix, layer['name'], counters[layer_name])
...
def main(args):
    ckpt_path = os.path.join(args.output, os.path.splitext(
        os.path.split(args.cfg)[-1])[0] + ".ckpt")
    pb_path = os.path.join(args.output, os.path.splitext(
        os.path.split(args.cfg)[-1])[0] + ".pb")
    ...
    parse_net(args.layers, args.cfg, args.weights,
              args.training, const_inits=False, verbose=False)
    graph = tf.get_default_graph()
    ...
    with tf.Session(graph=graph) as sess:
        ...
        saver.export_meta_graph(
            ckpt_path+'.meta', clear_devices=True, clear_extraneous_savers=True)
