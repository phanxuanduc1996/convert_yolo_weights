...
from keras.layers import Conv2D, DepthwiseConv2D, Dense, GlobalAveragePooling2D, GlobalMaxPooling2D, Flatten, Softmax, Dropout, ZeroPadding2D
from keras.layers import BatchNormalization, Add, Multiply, Reshape
from keras.layers import Input, Activation, ReLU, Reshape, Lambda
...
def MobileNetV3Small(input_shape=None,
    ...
    def stack_fn(x, kernel, activation, se_ratio):
        ...
        x = _inverted_res_block(x, 4, depth(40), kernel,
                                2, se_ratio, activation, 3)
        x = _inverted_res_block(x, 6, depth(40), kernel,
                                1, se_ratio, activation, 4)
        x = _inverted_res_block(x, 6, depth(40), kernel,
                                1, se_ratio, activation, 5)
        x = _inverted_res_block(x, 3, depth(48), kernel,
                                1, se_ratio, activation, 6)
        x = _inverted_res_block(x, 3, depth(48), kernel,
                                1, se_ratio, activation, 7)
        x = _inverted_res_block(x, 6, depth(96), kernel,
                                2, se_ratio, activation, 8)
        x = _inverted_res_block(x, 6, depth(96), kernel,
                                1, se_ratio, activation, 9)
        x = _inverted_res_block(x, 6, depth(96), kernel,
                                1, se_ratio, activation, 10)
        return x
...
def MobileNetV3Large(input_shape=None,
    ...
    def stack_fn(x, kernel, activation, se_ratio):
        ...
        x = _inverted_res_block(x, 6, depth(112), 3, 1,
                                se_ratio, activation, 10)
        x = _inverted_res_block(x, 6, depth(112), 3, 1,
                                se_ratio, activation, 11)
...
if __name__ == '__main__':
    ...
    model = MobileNetV3Large(
        include_top=True, input_tensor=input_tensor, weights='imagenet', alpha=1.0)
    model.summary()
